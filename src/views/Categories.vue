    ...
    <div class="d-flex">
        ...
        <div class="content-categories">
            <ul class="filter-category" v-for="(list, index) in categories" :key="index">
                ...
                <svg id="Layer_1" enable-background="new 0 0 512 512" height="20" viewBox="0 0 512 512" width="20"
                ...
                <svg height="20" viewBox="0 0 512 511" width="20" @click="openEditCat(list)">
                ...
                <list-category :data="list" :category-id="list.categoryId"></list-category>

            </ul>
            <p @click="openAddCat(categories)"> Add new category</p>
        </div>
        <add-new-category v-if="addCategoryStatus"/>
        <edit-category-modal v-if="editCategoryStatus"/>
    </div>
</template>
...
<script>
  ...
  export default {
    name: "Categories",
    components: {EditCategoryModal, AddNewCategory, ListCategory, DashboardNav},
    data(){
      return{
        rout:''
      }
    ...
    computed: {
      ...mapState(['categories', 'addCategoryStatus', 'editCategoryStatus'])
    },
    created() {
    ...
    methods: {
      openAddCat(){
        this.$store.commit('setAddCategoryModal', true)
      },
      openEditCat(){
        this.$store.commit('setEditCategoryModal', true)
      }
    }
  }
</script>
